...
const AboutSection = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      floatTimeline.current = gsap.timeline({
        repeat: -1,
  ...
  useEffect(() => {
    if (floatTimeline.current) {
      if (isHovered) {
        floatTimeline.current.pause();
      } else {
        floatTimeline.current.resume();
      }
    }
  }, [isHovered]);
          ...
          <div ref={contentRef} className="space-y-12">
            ...
            <div className="space-y-8">
              {features.map((feature, index) => (
                <div
                  key={index}
                  className="flex items-start space-x-6 p-0 group opacity-0 translate-y-8"
                  style={{ animation: `fadeInUp 0.7s ${0.2 + index * 0.15}s forwards` }}
                >
                  <div className="w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 bg-gradient-to-br from-primary via-fuchsia-400 to-cyan-400 animate-gradient-move">
          ...
          <div ref={imageRef} className="relative flex justify-center items-center">
            <div className="relative rounded-3xl overflow-hidden shadow-2xl bg-none">
              <img 
                ...
                src={aboutImage} 
                alt="About SPLOSH" 
                className={`w-full h-auto object-contain drop-shadow-2xl transition-all duration-300 ${isHovered ? 'ring-4 ring-primary/60 scale-105' : ''}`}
