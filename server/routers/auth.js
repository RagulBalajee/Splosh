...
router.get("/all-data", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const data = await registration.find(filter)
      ...
      if(idstatus){
      if (idstatus === 'below') {
          filteredData = data.filter(item => ((item.totalIncome / item.return) * 100) < 25);
      } else if (idstatus === 'half_done') {
          filteredData = data.filter(item => {
              const percentage = (item.totalIncome / item.return) * 100;
              return percentage >= 25 && percentage <= 50;
          });
      } else if (idstatus === 'beyond_half') {
          filteredData = data.filter(item => ((item.totalIncome / item.return) * 100) > 50);
      } else {
          filteredData = data; // If no idstatus provided, return all data
      }
    } else {
      filteredData = data;
    }
...
router.post("/user-block", verifyToken, async (req, res) => {
  try {
    const { user, status } = req.body;
    console.log(user, status);
